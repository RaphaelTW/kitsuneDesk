...
test('métricas de abertura são opt-in e normalizam durações', () => {
  let enabled = false;
  let inserted = null;
  const database = {
    get(sql) {
  ...
  assert.deepEqual(repository.recordStartup(1, { shellReadyMs: 100, coreReadyMs: 400 }), {
    recorded: false,
    reason: 'disabled'
  });
  ...
  assert.deepEqual(inserted, [1, 126, 126, 1]);

  const summary = repository.startupSummary(1);
