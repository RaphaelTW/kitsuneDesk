...
expect(
  /license:\s*docs\/INSTALLER_TERMS\.txt/,
  builder,
  'Instalador precisa exibir termos obrigatórios.'
);
expect(
  /oneClick:\s*false/,
  builder,
  'Instalador precisa usar modo assistido com aceite explícito.'
);
expect(
...
expect(
  ...
  terms,
  'Termos precisam declarar que o app não hospeda conteúdo.'
);
expect(/concorda|agree/i, terms, 'Termos precisam mencionar aceite do usuário.');
expect(
  /PROVIDER_MANIFEST_PRIVATE_KEY/,
  workflow,
  'Workflow precisa assinar manifesto dos provedores offline.'
);
expect(
  /Get-AuthenticodeSignature/,
  workflow,
  'Workflow precisa validar Authenticode do instalador.'
);
expect(/ValidateRollback/, workflow, 'Workflow precisa validar rollback instalado.');
expect(
  /ValidateInterruptedDownload/,
  workflow,
  'Workflow precisa validar recuperação após download interrompido.'
);
